...
import {ZapMemeUSDGVault} from "../src/ZapMemeUSDGVault.sol";
import {ZapTestWETH as MemeZapTestWETH} from "./LPZapRouter.t.sol";
...
contract MemeUSDGZapTest is BaseTest, V4Fixture {
    ZapDealVault internal zvault;
    LPZapRouter internal zap;
    MemeZapTestWETH internal weth;
    ...
    function setUp() public override {
        super.setUp();
        _deployV4();
        weth = new MemeZapTestWETH();
        zvault = ZapDealVault(address(new ZapMemeUSDGVault(usdg, registry, address(feeSink), address(posm), GRACE)));
        target = _initPoolAtPrice(address(meme), 1e18, address(usdg), 100e6, address(0));
    ...
    function test_memeStockCannotEnterMemeUSDGVault() public {
        uint256 nft = _mintFullRange(memeTarget, 1e12, borrower, borrower);
        vm.startPrank(borrower);
        IERC721(address(posm)).approve(address(zvault), nft);
        vm.expectRevert();
        zvault.list(Collateral(Kind.UNIV4_POSITION, address(posm), nft), 100, T7, 0, 90);
        vm.stopPrank();
        assertEq(IERC721(address(posm)).ownerOf(nft), borrower);
        assertEq(zvault.dealCount(), 0);
    }

    function test_revokingMemePairPreventsNewZapButPreservesWithdrawal() public {
        (uint256 id, uint256 nft) = _zap();
        vm.prank(safe);
