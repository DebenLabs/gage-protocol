...
import {ERC20Adapter} from "./libraries/ERC20Adapter.sol";
...
///         Holds all collateral and all USDG. No owner, no admin path to user funds, no oracle, no upgrade.
...
contract DealVault is IDealVault, IERC721Receiver, ReentrancyGuardTransient {
    ...
    function _pullCollateral(Collateral calldata c) internal virtual {
        if (c.kind == Kind.ERC20) {
            ICollateralRegistry.ERC20Config memory cfg = REGISTRY.getERC20Config(c.token);
            ERC20Adapter.validate(cfg, c.token, c.amountOrTokenId, openRaw[c.token]);
            openRaw[c.token] += c.amountOrTokenId;
            ERC20Adapter.pullExact(IERC20(c.token), msg.sender, c.amountOrTokenId);
        } else {
