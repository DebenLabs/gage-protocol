...
import {SeedTimelock} from "./SeedTimelock.sol";

/// @notice One-use seed transaction: initialize, mint, deploy the 365-day lock, lock, refund rounding dust.
/// @dev Only the deployment EOA can choose the pool and amounts. No publicly empty timelock is ever exposed.
contract SeedBootstrap is IERC721Receiver, ReentrancyGuardTransient {
    ...
    function seed(PoolKey calldata key, uint160 sqrtPriceX96, uint128 amount0, uint128 amount1) external nonReentrant {
        ...
        SeedTimelock lock = new SeedTimelock(POSM, OWNER);
        timelock = lock;
