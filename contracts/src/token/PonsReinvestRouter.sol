...
contract PonsReinvestRouter is IReinvestRouter, PonsV4Swapper, ReentrancyGuardTransient {
    ...
    constructor(Params memory p, address curve_, uint24 launchPoolFeePips_)
        PonsV4Swapper(p.poolManager, curve_, p.gageEth, launchPoolFeePips_)
    {
    ...
    function _ethForGage(uint256 gageOut) internal view returns (uint256 ethIn) {
        if (!PONS_CURVE.graduated()) return _curveEthForGage(gageOut);
        // Reverse exact-input quotes subtract fees; an exact-output budget must add them.
        uint256 netSellQuote = _quoteAtSpot(_gageEth, false, gageOut, 0, 0);
        uint256 kept = 1_000_000 - PONS_HOOK_FEE_PIPS;
        ethIn = Math.mulDiv(netSellQuote, 1_000_000 * 1_020_000, kept * kept, Math.Rounding.Ceil);
    }
    ...
    function _amount0(uint160 sqrtA, uint160 sqrtB, uint128 liquidity) internal pure returns (uint256) {
        return PonsSqrtPriceMathLib.amount0RoundUp(sqrtA, sqrtB, liquidity);
    }

    function _amount1(uint160 sqrtA, uint160 sqrtB, uint128 liquidity) internal pure returns (uint256) {
        return PonsSqrtPriceMathLib.amount1RoundUp(sqrtA, sqrtB, liquidity);
    }
}

import {SqrtPriceMath} from "@uniswap/v4-core/src/libraries/SqrtPriceMath.sol";

library PonsSqrtPriceMathLib {
