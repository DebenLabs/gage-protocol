...
contract GageV2ZapRouter is V4Swapper, ReentrancyGuardTransient {
    ...
    constructor(
        ...
        if (
            address(weth) == address(0) || address(permit2) == address(0)
                || vault.VALIDATOR().V4_MANAGER() != address(posm) || address(vault.USDG()) == address(weth)
        ...
        returns (uint256 dealId, uint256 tokenId, uint128 liquidity)
    {
        ...
        dealId = VAULT.listFor(
            Collateral(Kind.UNIV4_POSITION, address(POSM), tokenId),
            z.minPrice,
            z.cap,
            z.term,
            z.fundingDeadline,
            z.minimumQuotedRewards,
            msg.sender
        );
        _refund(b);
    ...
    function _validate(Zap calldata z, uint256 swaps) private view {
        if (block.timestamp >= z.deadline || z.deadline > block.timestamp + 15 minutes) revert Expired();
        if (
